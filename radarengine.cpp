...
RadarEngine::RadarEngine *RadarEngine::RadarEngine::GetInstance(QObject *parent)
{
    if (m_instance == nullptr)
        m_instance = new RadarEngine(parent);
    return m_instance;
}

RadarEngine::RadarEngine::RadarEngine(QObject *parent) : QObject(parent)
{
    qDebug() << Q_FUNC_INFO;
...
void RadarEngine::RadarEngine::onRadarConfigChange(QString key, QVariant val)
{
    ...
    if (key == NON_VOLATILE_PPI_DISPLAY_LAST_SCALE)
        ...
        guardZones["GZ 1"]->SetInnerRange(val.toInt());
    else if (key == NON_VOLATILE_GZ_START_RANGE1)
        guardZones["GZ 2"]->SetInnerRange(val.toInt());
    else if (key == NON_VOLATILE_GZ_START_BEARING)
        guardZones["GZ 1"]->SetStartBearing(val.toDouble());
    else if (key == NON_VOLATILE_GZ_START_BEARING1)
        guardZones["GZ 2"]->SetStartBearing(val.toDouble());
    else if (key == NON_VOLATILE_GZ_END_RANGE)
        guardZones["GZ 1"]->SetOutterRange(val.toInt());
    else if (key == NON_VOLATILE_GZ_END_RANGE1)
        guardZones["GZ 2"]->SetOutterRange(val.toInt());
    else if (key == NON_VOLATILE_GZ_END_BEARING)
        guardZones["GZ 1"]->SetEndBearing(val.toDouble());
    else if (key == NON_VOLATILE_GZ_END_BEARING1)
        guardZones["GZ 2"]->SetEndBearing(val.toDouble());
    else if (key == NON_VOLATILE_GZ_MODE)
        guardZones["GZ 1"]->SetType(static_cast<GZType>(val.toInt()));
    else if (key == NON_VOLATILE_GZ_MODE1)
        guardZones["GZ 2"]->SetType(static_cast<GZType>(val.toInt()));
    else if (key == NON_VOLATILE_PPI_DISPLAY_SHOW_GZ)
        guardZones["GZ 1"]->SetShown(val.toBool());
    else if (key == NON_VOLATILE_PPI_DISPLAY_SHOW_GZ1)
        guardZones["GZ 2"]->SetShown(val.toBool());
}
...

{
    ...
    const bool is_trail_enable = RadarConfig::getInstance("")->getConfig(NON_VOLATILE_RADAR_TRAIL_ENABLE).toBool();
    const double currentHeading = heading_up ? 0 : RadarConfig::getInstance("")->getConfig(NON_VOLATILE_NAV_DATA_LAST_HEADING).toDouble();
    ...
    short int hdt_raw = static_cast<short>(SCALE_DEGREES_TO_RAW(headingCorrection));
    ...
    for (int i = 0; i <= UINT8_MAX; i++)
    {
        colourMap[i] = (i >= 200 /*red strong threshold*/) ? BLOB_STRONG
                       : (i >= 100 /*green strong threshold*/)
                           ? BLOB_INTERMEDIATE
                       : (i >= 50 /*blue strong threshold*/) ? BLOB_WEAK
                                                             : BLOB_NONE; // next implementation load from conf file
    }
