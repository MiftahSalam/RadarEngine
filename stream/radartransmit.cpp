...
void RadarTransmit::SetRange(int meters)
{
    ...
    if (meters >= 50 && meters <= 120008704)
    {
        ...
        socket.writeDatagram((const char*)&pck,6,QHostAddress(m_data), m_data_port);
...
void RadarTransmit::RadarStby()
{
    ...
    socket.writeDatagram((const char*)&standby1,3,QHostAddress(m_data), m_data_port);
    socket.waitForBytesWritten();
    socket.writeDatagram((const char*)&standby2,3,QHostAddress(m_data), m_data_port);
...
void RadarTransmit::RadarTx()
{
    ...
    socket.writeDatagram((const char*)&transmit1,3,QHostAddress(m_data), m_data_port);
    socket.waitForBytesWritten();
    socket.writeDatagram((const char*)&transmit2,3,QHostAddress(m_data), m_data_port);
...
void RadarTransmit::RadarStayAlive()
{
    ...
    socket.writeDatagram((const char*)&transmit1,2,QHostAddress(m_data), m_data_port);
    socket.writeDatagram((const char*)&transmit2,2,QHostAddress(m_data), m_data_port);
    socket.writeDatagram((const char*)&transmit3,2,QHostAddress(m_data), m_data_port);
    socket.writeDatagram((const char*)&transmit4,2,QHostAddress(m_data), m_data_port);
...
void RadarTransmit::SetControlValue(ControlType controlType, int value) {  // sends the command to the radar
    ...
    case CT_BEARING_ALIGNMENT:
    {
        ...
        socket.writeDatagram((const char *)cmd,4,QHostAddress(m_data), m_data_port);
        break;
    }
    case CT_GAIN:
    {
        ...
        {  // AUTO gain
            ...
            socket.writeDatagram((const char *)cmd,11,QHostAddress(m_data), m_data_port);
        }
        else
        {  // Manual Gain
    ...
    case CT_INTERFERENCE_REJECTION:
    {
        ...
        socket.writeDatagram((const char *)cmd,3,QHostAddress(m_data),m_data_port);
        break;
    }
    ...
    case CT_ANTENNA_HEIGHT:
    {
        ...
        socket.writeDatagram((const char *)cmd,10,QHostAddress(m_data),m_data_port);
        break;
    }
    }
}
