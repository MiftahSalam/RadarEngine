...
QStringList RadarConfig::nonVolatileKeys =
    QStringList() << NON_VOLATILE_PPI_DISPLAY_UNIT
                  ...
                  << NON_VOLATILE_ARPA_NET_CONFIG_PUBLIC
                  ...
                  << NON_VOLATILE_NAV_NET_CONFIG_PUBLIC
...
QVariant RadarConfig::getConfig(const QString &key) const
{
    QVariant val;
    if (volatileVar.contains(key))
        val = volatileVar.value(key);
    else if (nonVolatileVar.contains(key))
        val = nonVolatileVar.value(key);
...
void RadarConfig::loadConfig()
{
    ...
    while (i < nonVolatileVar_keys.size())
    {
        ...
        if (!buf_nonVolatileKeys.contains(nonVolatileVar_keys.at(i)))
            qWarning() << Q_FUNC_INFO << "undesired key" << nonVolatileVar_keys.at(i);
        else
            buf_nonVolatileKeys.removeOne(nonVolatileVar_keys.at(i));
        i++;
    }
...
void RadarConfig::initConfig()
{
    ...
    nonVolatileVar.insert(NON_VOLATILE_ARPA_NET_CONFIG_PUBLIC, "mqtt;InOut;127.0.0.1:1883:user:pass:arpa:5");
    ...
    nonVolatileVar.insert(NON_VOLATILE_NAV_NET_CONFIG_PUBLIC, "mqtt;InOut;127.0.0.1:1883:user:pass:site_data");
}
...
{
    if (instance == nullptr)
        instance = new RadarConfig(nullptr, path);
    else
    {
        if (instance->filePath != path && !path.isEmpty())
            return nullptr;
    }
